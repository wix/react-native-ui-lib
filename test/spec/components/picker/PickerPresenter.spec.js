...
describe('components/PickerPresenter', () => {
  it('should isItemSelected, handle single mode', () => {
    expect(uut.isItemSelected('value', {value: 'value', label: 'label'})).toBe(true);
    expect(uut.isItemSelected('value1', {value: 'value2'})).toBe(false);
  });

  it('should isItemSelected, handle multi mode', () => {
    expect(uut.isItemSelected('value', [{value: 'value', label: 'label'}])).toBe(true);
    expect(uut.isItemSelected('value', [{value: 'value'}, {value: 'value1'}, {value: 'value2'}])).toBe(true);
    expect(uut.isItemSelected('value1', [{value: 'value2'}])).toBe(false);
