...
### Safe Area Context
If you are using `react-native-safe-area-context` and want to use `useSafeAreaInsets()` in your app, make sure to wrap your root component with `SafeAreaProvider`:
...
Note: Modal and Dialog components automatically wrap their content with `SafeAreaProvider` when the package is installed, so `useSafeAreaInsets()` works inside modals without additional setup.
