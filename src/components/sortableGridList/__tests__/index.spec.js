...
const TEST_DATA = Array(9).fill(0).map((_, index) => ({
  id: `${index}`
}));
const itemsTestId = id => `${testID}.item.${id}`;
const testRenderItem = ({
  item
}) => {
  return <View height={50} testID={itemsTestId(item.id)}>
      <Text>{item.id}</Text>
    </View>;
};
...
describe('SortableGridlist', () => {
  ...
  it('should reorder by index', () => {
    ...
    const driverItem = useDraggableDriver(useComponentDriver({
      renderTree,
      testID: itemsTestId('0')
    }));
    ...
    expect(onOrderChange).toHaveBeenCalledWith(newOrder.map(index => TEST_DATA[index]), newOrder.map(index => TEST_DATA[index].id));
  });
