...
describe('Incubator.Dialog sanity checks', () => {
  ...
  it('Should dismiss dialog on background press', () => {
    ...
    const {
      dialogDriver
    } = getDriver(<TestCase1 visible onDismiss={dismissFn} />);
  ...
  it('Should exist only if visible', () => {
    ...
    expect(dialogDriver.isVisible()).toBeFalsy();
    const openButtonDriver = ButtonDriver({
      renderTree,
      testID: 'openButton'
    });
    ...
    const closeButtonDriver = ButtonDriver({
      renderTree,
      testID: 'closeButton'
    });
