...
import ReactNative, { NativeModules, StyleSheet, Image, DeviceEventEmitter } from "react-native";
...
export default class TagsInput extends BaseComponent {
    ...
    renderTag(tag, index) {
        ...
        return (<View key={index} style={[styles.tag, tagStyle, shouldMarkTag && styles.tagMarked]}>
        {this.renderLabel(tag, shouldMarkTag)}
      </View>);
    }
    renderTagWrapper(tag, index) {
        return (<TouchableOpacity key={index} activeOpacity={1} onPress={() => this.onTagPress(index)}>
