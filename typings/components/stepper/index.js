...
export default class Stepper extends BaseComponent {
    ...
    render() {
        ...
        <View style={this.styles.title}>
          <Text testID={"label"} style={this.styles.titleText}>
            {this.getLabel()}
          </Text>
        </View>
        <View style={this.styles.buttons}>
          <StepperButton label="-" testId={"decrease"} styles={this.styles} disabled={minusDisabled} onPress={() => this.updateValue(this.state.value - 1)}/>
          <View style={this.styles.separator}/>
          <StepperButton label="+" testId={"increase"} styles={this.styles} disabled={plusDisabled} onPress={() => this.updateValue(this.state.value + 1)}/>
        </View>
      </View>);
    }
}
