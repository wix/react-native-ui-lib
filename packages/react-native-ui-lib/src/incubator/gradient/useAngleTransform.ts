...
function getStartEndFromAngle(angle = 0) {
  ...
  const end = {x: cx + vx * t, y: cy + vy * t};
  const start = {x: cx - vx * t, y: cy - vy * t};
  ...
  return {
    start: {x: quantize(start.x), y: quantize(start.y)},
    end: {x: quantize(end.x), y: quantize(end.y)}
  };
}

export const _forTesting = {getStartEndFromAngle}; // exporting private functions for testing only

export type AngleTransformProps = {
  angle?: number;
};

const useAngleTransform = (props: AngleTransformProps) => {
  const {angle} = props;
